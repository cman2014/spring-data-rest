...
public class ExposureConfiguration implements ExposureConfigurer {
	private ComposableFilter<ResourceMetadata, ConfigurableHttpMethods> collection = AggregateResourceHttpMethodsFilter.none();
	private ComposableFilter<ResourceMetadata, ConfigurableHttpMethods> item = AggregateResourceHttpMethodsFilter.none();
	private ComposableFilter<PropertyAwareResourceMapping, ConfigurableHttpMethods> property = AssociationResourceHttpMethodsFilter.none();
	...
	private static <T> ComposableFilter<ResourceMetadata, ConfigurableHttpMethods> withTypeFilter(Class<?> type, ComposableFilter<ResourceMetadata, ConfigurableHttpMethods> function) {
		...
		//
		type.isAssignableFrom(metadata.getDomainType()) ? function.filter(metadata, httpMethods) : httpMethods;
	}
	...
	private class TypeBasedExposureConfigurer implements ExposureConfigurer {
		...
		public ExposureConfigurer withAssociationExposure(AssociationResourceHttpMethodsFilter filter) {
			...
			(mapping, methods) ->  //
			type.isAssignableFrom(mapping.getProperty().getOwner().getType()) ? filter.filter(mapping, methods) //
			 : methods;
		...
		public TypeBasedExposureConfigurer(final Class<?> type) {
			this.type = type;
		}
	}
}
